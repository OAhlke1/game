class Char {
    ...
    jumpingIntervalStep;
    floorPosition;
    headJumpAmount = 50; /** @var {number} headJumpAmount is the amount that the char takes away from an enemy when jumping on its */
    hitImagesAmount = 7; /** @var {number} hitImagesAmount is the amount of images of the chars animation when it gets hit */
    bulletAmount = 10; /** @var {number} bulletAmount is the amount that a bullet takes away from an enemies health amount */
    maxHealthAmount = 200; /** @var {number} maxHealthAmount is the chars maximum health amount */
    scrollingStepAmount = 0; /** @var {number} scrollingStepAmount is the amount of steps the char has shifted the canvas to the left while walking */
    hittingAnimationIndex = 0; /** @var {number} hittingAnimationIndex is the actual index of the animation when the char got hit */
    hittingAnimationStep = 0; /** @var {number} hittingAnimationStep is the amount of cycles the animation when the char got hit is been going through */
    stepAmount = 0; /** @var {number} stepAmount is the amount of steps the enemy has been making ????????????????????????????? */
    totalStepAmount = 0; /** @var totalStepAmount is the entire amount of all steps of the char ????????????????????????????? */
    enemiesKilled = 0; /** @var {number} enemiesKilled is the amount of defeated enemies */
    atWallLeft = false; /** @var {boolean} atWallLeft says wether the char stands left at a wall */
    atWallRight = false; /** @var {boolean} atWallRight says wether the char stands right at a wall */
    jumps = false; /** @var {boolean} jumps says wether the char jumps or not */
    isAlive = true; /** @var {boolean} says wether the char is alive or not */
    sleeps = false; /** @var {boolean} sleeps says wether the char sleeps or not */
    gotHit = false; /** @var {boolean} gotHit says wether the char got hit by an enemy/trap or not */
    isImmune = false; /** @var {boolean} isImmune says wether the char is immune against attacks or not */
    landedOnPlatform = true; /** @var {boolean} landedOnPlatform says wether the char landed on a platform or not */
    targeted = false; /** @var {boolean} targeted says wether the char got hit by an enemy/trap or not */
    onUpwardsMovingPlatform = false; /** @var {boolean} onUpwardsMovingPlatform says wether the char is on an upwards moving platform or not */
    underGround = false; /** @var {boolean} underGround says wether the char is under the ground or not */
    movingDirection = 'right'; /** @var movingDirection is the direction the char is moving at */
    walkingFrameRate = 18; /** @var {number} walkingFrameRate is the frame-rate for the walking-animation */
    hasDoneAnything = false; /** @var {boolean} hasDoneAnything says wether the char has done anything yet or not */
    hitImagesArrays = {
        left: [],
        right: []
 ...
 */
constructor(width, height, x, y, stepLength, maxJumpHeight, specialAmmoParts = 0, healthAmount, onMovingPlatform, standingPlatformIndex) {
    this.width = width;
    this.height = height;
    this.x = x;
    ...
    this.charImage = new Image(); /** is the DOM-element of the char-image */
    this.charImage.src = `./graphics/main-char/run/run-${this.movingDirection}-0.png`;
...
setMovingState(key) {
    if (!gamePaused && this.isAlive && !keysBlockedForShifting) {
        ...
        this.charImage = this.runImagesArrays[this.movingDirection][Math.abs(this.stepAmount % 12)];
    }
}
...
jump() {
    ...
    if (!gamePaused && this.isAlive && !this.falls && this.landedOnPlatform) {
        ...
        this.stopFalling();
        this.stepAmount = 0;
        this.charImage = this.jumpingImages[this.movingDirection];
...
fall() {
    ...
    if (!gamePaused && this.isAlive) {
        if (!this.gotHit) {
            ...
            this.runImagesArrays[this.movingDirection][Math.abs(this.stepAmount % 12)];
        }
...
whenReachingPlatform() {
    ...
    this.charImage = this.runImagesArrays[this.movingDirection][0];
    this.startingYPos = null;
    this.jumps = false;
 ...
 */
animateHit() {
    if (this.gotHit && !gamePaused) {
        if (this.isAlive) { this.charImage = this.hitImagesArrays[this.movingDirection][this.hittingAnimationIndex % this.hitImagesAmount] }
...
/**
 * 
 * @method setImagePath sets the path for @this charImage
 * @param {string} path is the image-path
 */
setImagePath(path) {
    if (this.isAlive) {
        this.charImage.src = path;
    } else {
        this.charImage.src = `./graphics/main-char/dead/dead-${this.movingDirection}.png`;
    }
}
