...
async function resetGame() {
    await clearLocalStorage().then(async () => {
        await resetEnemies();
        resetChar();
        resetItems();
        unholdAllKeys();
        document.querySelector('.canvas-cont .game-over-screen').classList.add('disNone');
...
/**
 * @function resetItems resets all items.
 */
function resetItems() {
    items.lifeIncreasing.forEach((elem) => {
...
/**
 * @function resetEnemies resets all enemies.
 */
async function resetEnemies() {
    hitables.enemies.forEach((elem) => {
        elem.lifeAmount = elem.maxLifeAmount;
        elem.isAlive = true;
        elem.isDangerous = true;
        elem.x = elem.standardX;
        elem.y = elem.standardY;
        elem.walks = false;
        elem.checkCharPos();
    });
}
...
function startSavingChar() {
    if (char && !char.hasDoneAnything && loadedInWidescreen) {
...
function canContControlsToggle() {
    if (controlsBar.classList.contains('disNone')) {
        controlsBar.classList.remove('disNone');
        pauseGame();
    } else if (!controlsBar.classList.contains('disNone')) {
        controlsBar.classList.add('disNone');
        if (description.classList.contains('disNone')) {
            unpauseGame();
            return;
        } else if (!description.classList.contains('disNone')) { pauseGame(); }
    }
}
